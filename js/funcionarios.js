...
const Funcionarios = (() => {
  ...
  // ─── Formulário ───────────────────────────────────────────
  const _fields = ['id_display','nome','funcao','departamento','dataAdmissao',
                   'tipoContrato','salarioBase','telefone','email','morada','estado'];
  ...
  const _fillForm = (f) => {
    _fields.forEach(field => {
      ...
      if (!el) return;
      if (field === 'dataAdmissao') el.value = Utils.toInputDate(f[field]);
